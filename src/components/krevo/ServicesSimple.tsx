      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 gap-16 md:grid-cols-3 md:gap-20 lg:gap-24">
          {categories.map(({ title, icon: Icon, items }, index) => (
            ...
            >
              <span
                className="pointer-events-none absolute -top-4 left-1/2 -translate-x-1/2 select-none text-[96px] leading-none font-bold text-[#0052CC]/[0.04] md:left-0 md:translate-x-0"
                aria-hidden="true"
              >
              ...
              <div
                ...
                style={{
                  background:
                    "radial-gradient(ellipse at center, rgba(0,82,204,0.12) 0%, transparent 70%)",
                }}
                aria-hidden="true"
              />
              ...
              <div className="relative inline-flex items-center justify-center">
                <span
                  className="absolute h-16 w-16 rounded-full bg-[#0052CC]/10"
                  aria-hidden="true"
                />
                <Icon
                  size={48}
                  strokeWidth={1.4}
                  className="relative text-[#3399FF] transition-transform duration-300 group-hover:scale-110"
                />
              </div>
              ...
              <ul className="relative mt-5">
                {items.map((item) => (
                  <li
                    key={item}
                    className="border-b border-[#0052CC]/[0.08] py-3 first:pt-0"
                  >
                    <span className="flex items-start justify-center gap-2.5 text-[14px] text-krevo-silver md:justify-start">
                      <span
                        className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#3399FF]"
                        aria-hidden="true"
                      />
                      {item}
                    </span>
          ...
          {highlights.map(({ label, icon: Icon }) => (
            ...
            >
              <Icon
                size={16}
                strokeWidth={1.6}
                className="shrink-0 text-[#3399FF]"
                aria-hidden="true"
              />
              <span>{label}</span>
            </div>
        ...
        <div className="mt-10 flex justify-center md:mt-12">
          <a
            href="/servicii"
            className="inline-flex min-h-11 items-center rounded-full border border-[#0052CC] bg-transparent px-6 py-3 text-[14px] font-medium text-white transition-colors duration-200 hover:border-[#0066FF] hover:bg-[#0052CC]/10"
          >
